...
tasks_list = []  # global list is declared for storing all the task

counter = 1  # global variable is declared for counting the task
...
def insert_task():
    ...
    value = input_error()  # check for error

    if value == 0:  # if error occurs, then return
        return
    ...
    clear_task_field()  # function calling for deleting the content of task field
...
def delete():  # function for deleting the specified task
    ...
    clear_task_number_field()  # function calling for deleting the content of task number field
...
if __name__ == '__main__':
    ...
    gui.configure(background='light green')  # set the background color of a GUI window
    ...
    Submit = Button(gui, text='Submit', fg='Black', bg='Red', command=insert_task)

    # create a text area for the root with lucida 13-font text area is for writing the content
